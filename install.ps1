# Comprehensive installer for cognis-digital/geoaoi-pro (Windows PowerShell).
# Tries: pipx -> uv -> pip (git+https) -> from source.
# geoaoi-pro is source-available and not on PyPI; all paths install from GitHub.
$ErrorActionPreference = "Stop"
$Repo = "geoaoi-pro"
$Url  = "git+https://github.com/cognis-digital/geoaoi-pro.git"
$Git  = "https://github.com/cognis-digital/geoaoi-pro.git"
...
if (Have pipx) {
  ...
  pipx install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: geoaoi-pro"; exit 0 }
}
if (Have uv) {
  Say "Installing with uv..."
  uv tool install $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: geoaoi-pro"; exit 0 }
}
if (Have pip) {
  Say "Installing with pip (user site)..."
  pip install --user $Url; if ($LASTEXITCODE -eq 0) { Say "Done. Run: geoaoi-pro"; exit 0 }
}
