"""Native cognis-connect emit for geoaoi-pro — forward findings to any platform.

Maps geoaoi-pro's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    geoaoi-pro ... --format json | geoaoi-pro-emit --to stix
    geoaoi-pro-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('message', 'Unknown')
        out['severity'] = rec.get('level', 'info')
